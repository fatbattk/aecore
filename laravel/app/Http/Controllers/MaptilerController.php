...
class MaptilerController extends PlanroomController {
	 ...
	 * Class constructor.
	 */
	public function __construct($image_path, $s3path, $options = array())
	{
	...
	public function prepareZoomBaseImages($min = null, $max = null){
		...
		for($i = $this->zoom_max; $i >= $this->zoom_min; $i--){
			...
			//store
			$this->imageSave($lvl_image, $lvl_file, $tile_name);
	...
	public function tilesForZoom($zoom) {
      ...
      $x = $y;
		...
		for($ix = 0; $ix < $x; $ix++){
			...
			for($iy = 0; $iy < $y; $iy++){
				//full file path
				$tile_name = sprintf($this->store_structure, $zoom, $ix, $iy).'.'.$ext;
				...
				//save
				$this->imageSave($tile, $lvl_file, $tile_name);
	 ...
	 */
	protected function imageSave($image, $dest, $tile_name){
		//prepare folder
